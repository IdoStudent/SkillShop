    ...
    render(){
        return <div className="test">
            <SiteWrapper>
              ...
              <Header.H1 className="pageHeading">
                Your Matches
              </Header.H1>
              <div className="my-body">
                <Container className="card">
                  ...
                  <Grid.Row className="row">
                    {/* JobSeeker Search */}
                    {this.state.userType == "jobseeker" && <Grid.Col className="col-3 search">
                        <input className="input-text-search" type="text" placeholder="Search" value={this.state.search} onChange={this.handleSearchChange} />
                      </Grid.Col>}
                    {/* Employer drop down menu */}
                    {this.state.userType == "employer" && <Grid.Col className="col-3 search">
                        <Form.Select id="profile" onChange={this.handleDropDownMenu}>
                          {this.state.jobTitles.map((jobTitle) => {
                            return <option value={jobTitle.jobKey}>
                                {jobTitle.jobTitle}
                              </option>;
                          })}
                        </Form.Select>
                  ...
                  <Grid.Row className="row">
                    {/* List */}
                    {this.state.search == "" && this.state.userType == "jobseeker" && <Grid.Col className="col-3 list">
                          <ul className="emp-list">
                            {this.state.jobKeys.map((employer) => {
                              return <li key={employer.id} className="emp-item">
                    ...
                    {this.state.search != "" && this.state.userType == "jobseeker" && <Grid.Col className="col-3 list">
                          <ul className="emp-list">
                            {this.state.jobKeys
                              .filter((k) => this.state.employersNames
                                    .filter((em) => em.key == this.state.employersEmails.filter((emp) => emp.key == k.key)[0].email)[0]
                                    .name.toLowerCase()
                    ...
                    {/* employer side list */}
                    {this.state.loading == false ? this.state.userType == "employer" ? this.state.currentPosition !== "" ? <Grid.Col className="col-3 list">
                            <ul className="emp-list">
                          ...
                          </Grid.Col> : <Grid.Col className="col-3 list">
                            <ul className="emp-list" />
                          </Grid.Col> : console.log("waiting...") : <div className="alert">
                        loading...
                      </div>}
                    {/* Chat */}
                    <Grid.Col className="col-9">
                      ...
                      <Grid.Row className="row chat-box">
                        <ul className="message-list">
                          {this.state.loading == false ? (this.state.currentMatchID !== "" ? this.state.messages
                                  .filter(
                                    (m) =>
                                      m.matchID ==
                                  ...
                                  }) : console.log("waiting...")) : console.log("loading...")}
                        </ul>
                      </Grid.Row>
                      {/* Input */}
                      <Grid.Row className="row text-box">
                        <Grid.Col className="col-9 col-sm-10 col-md-10 col-lg-11">
                          <Form.Textarea className="input-text" type="text" placeholder="Type message here..." name="message" value={this.state.message} onChange={this.handleMessageChange} />
                        </Grid.Col>
                        <Grid.Col className="col-3 col-sm-2 col-md-2 col-lg-1">
                          <button className="my-button fa fa-send-o" onClick={this.handleMessageSubmit} />
                        </Grid.Col>
                      </Grid.Row>
                    </Grid.Col>
                  </Grid.Row>
