...
class Signup extends Component {
    ...
    if (this.validateForm()) {
      ...
      try {
        ...
        if (error.code === "UsernameExistsException") {
          this.setState({
            emailErrMsg: "An account with this email already exists",
            emailErr: true,
          });
        }
      }
  ...
  validateForm = () => {
    ...
    if (!password) {
      ...
      this.setState({
        passwordErrMsg: "Password needs to be atleast 8 characters",
        passwordErr: true,
      });
      validInput = false;
    }
    ...
    if (!passwordConfirm) {
      this.setState({
        passwordConfErrMsg: "Please confirm your password",
        passwordConfErr: true,
      });
      validInput = false;
    } else if (password !== passwordConfirm) {
      this.setState({
        passwordConfErrMsg: "Passwords do not match",
        passwordErr: true,
        passwordConfErr: true,
      });
      validInput = false;
        ...
        <Container className="wrapper">
          ...
          {this.state.validRegistration === false ? ( // Display if user has not signed up
            ...
            <Container className="card success">
              ...
              <p>
                {" "}
                Before you can login, you need to confirm your email. <br />{" "}
                <br /> To do that, find the email sent by us in your inbox and
                simply click the link. Once you've done that, you can click the
